  ...
  return (
    <div className="min-h-screen bg-black flex overflow-hidden" style={{ perspective: '1200px' }}>
      ...
      >
        ...
        <div className="relative z-10">
          <p className="text-white/30 text-xs font-semibold tracking-widest uppercase mb-4">Your Career Intelligence</p>
          <h2 className="text-5xl font-black text-white leading-tight mb-6">
            Welcome<br />
            <span className="text-white/30">back.</span>
          </h2>
          <p className="text-white/40 text-sm leading-relaxed max-w-xs mb-10">
            Track skills, prepare for interviews, and map your path to the role you want.
          </p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { num: '6+', label: 'Job Platforms' },
              { num: 'AI', label: 'Powered Engine' },
              { num: '100%', label: 'Free Forever' },
              { num: 'Live', label: 'Job Matching' },
            ].map((stat) => (
              <div key={stat.label} className="bg-white/5 border border-white/10 rounded-xl p-4">
                <p className="text-white font-black text-2xl">{stat.num}</p>
      ...
      >
        <div className="flex gap-1 bg-white/5 border border-white/10 rounded-xl p-1 mb-10 w-fit">
          ...
          <Link to="/register" className="text-white/40 hover:text-white text-sm font-medium px-5 py-2 rounded-lg transition">
            Register
          </Link>
        </div>
        ...
        <div className="max-w-sm">
          ...
          <p className="text-white/30 text-sm mb-8">Sign in to continue your career journey</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-white/30 text-xs hover:text-white cursor-pointer transition">
                Forgot password?
              </span>
            </div>
          ...
          <p className="text-white/20 text-xs text-center mt-8">
            New here?{' '}
            <Link to="/register" className="text-white/50 hover:text-white underline transition">
              Create an account ↗
            </Link>
          </p>
        </div>
